...
function defaultCalloutTitle(name) {
    return { note: 'Note', tip: 'Tip', warning: 'Heads up', danger: 'Careful' }[
        name
    ];
}
...
md.renderer.rules.link_open = (tokens, idx, options, env, self) => {
    ...
    if (href.startsWith('/') && !href.startsWith('//')) {
        ...
        token.attrSet(
            'href',
            `${BASE}${trimmed}${trimmed && !trimmed.endsWith('/') ? '/' : ''}${hash ? `#${hash}` : ''}`,
        );
...
function renderCode(code, language) {
    return USE_TORCHLIGHT
        ? torchlightPlaceholder(code, language)
        : renderLocally(code, language);
}
    ...
    return navPages.map(entry => {
        ...
        return {
            ...
            title:
                entry.title ??
                data.title ??
                (heading ? heading[1].trim() : entry.url),
...
function runTorchlight() {
    ...
    if (!fs.existsSync(binary)) {
        console.warn(
            '  ! Torchlight CLI not installed — run `npm install` in site/.',
        );
        return false;
    }
    ...
    const result = spawnSync(
        ...
        [
            '--config',
            path.join(here, 'torchlight.config.cjs'),
            '--input',
            outDir,
            '--output',
            outDir,
...
function build() {
    ...
    const sections = navSections.map(section => ({
        ...
        pages: section.pages.map(entry =>
            pages.find(page => page.file === entry.file),
        ),
    }));
    ...
    write(
        ...
        `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${pages
            .map(
                page =>
                    `  <url><loc>${site.origin}${BASE}${page.url}${page.url ? '/' : ''}</loc></url>`,
            )
            .join('\n')}\n</urlset>\n`,
    );
    write(
    ...
    for (const [file, contents] of aiEndpoints({
        pages,
        sections,
        site,
        version,
...
function serve(port = 4321) {
    http.createServer((request, response) => {
        ...
        const relative = url.startsWith(BASE)
