...
- `entries:` are paths inside the asset directory, validated at declaration. Declaring any registers four global Blade directives keyed by the package short name: `@packageAssets(?$package, ...$only)`, `@packageStyles(…)`, `@packageScripts(…)` and `@packageAssetUrl($package, $entry)` (URL only, both arguments required). `.js` renders as `type="module"` with `data-navigate-track="reload"`; pass `Support\Asset::make('js/x.js')->classic()` for an IIFE/UMD bundle, or `->attributes([...])` / `->asStylesheet()` for the rest.
